// A tls cert auto signer
// from github.com/dstotijn/hetty
...
// bytes (2^(8*20)-1).
var MaxSerialNumber = big.NewInt(0).SetBytes(bytes.Repeat([]byte{255}, 19))
...
func LoadOrCreateCA(caKeyFile, caCertFile string) (*x509.Certificate, *rsa.PrivateKey, error) {
	...
	caCert, caKey, err := NewCA("Lunar Rails bookqaq CA Certificate", "Lunar Rails bookqaq CA", 10*365*24*time.Hour)
	if err != nil {
